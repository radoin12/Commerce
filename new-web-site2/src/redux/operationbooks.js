...
//  show all products new here ff
  export const postPro=createAsyncThunk('product/postPro',async(currentPage,thunkApi)=>{
     const{rejectWithValue}=thunkApi
     try {
       const {data}=await  axios.get(`http://localhost:3000/displayBook?page=${currentPage}`)
...
export const updateProducts=createAsyncThunk('product/updateProducts',async(values)=>{
 ...
 const ProductReducer=createSlice({
  ...
  .addCase(postPro.fulfilled,(state,action)=>{
    ...
    return{
        ...state,productData:action.payload.books,totalPage:action.payload.totalPages,
        statusPro:'success'
